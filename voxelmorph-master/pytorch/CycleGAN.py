# -*- coding: utf-8 -*-
"""
Created on Mon Oct  5 14:31:53 2020
...
class Unet(nn.Module):
    ...
    """
    def __init__(self, enc_nf, dec_nf, volsize):
        """
        ...
        self.ST = SpatialTransformer(volsize)
...
class Dis_Unet(nn.Module):
    def __init__(self, volsize):
        ...
        layers = []
        depth = volsize[2] # get the 3rd dimension
        reduction_times = math.floor(math.log(depth,2))
        for i in range(reduction_times):
            if i == 0:
                layers.append(conv_block_dis(3,1,32,2))
            else:
                layers.append(conv_block_dis(3,32*i,32*(i+1),2))
        self.model = nn.Sequential(*layers)
        self.sig = nn.Sigmoid()
    def forward(self, img):
        output_FCN = self.model(img)
        output_flatten = output_FCN.view(-1)
        output_mean = torch.mean(output_flatten)
        output_sig = self.sig(output_mean)
        return output_sig
...
class conv_block_dis(nn.Module):
    """
    [conv_block] represents a single convolution block in the Unet which
    is a convolution based on the size of the input channel and output
    channels and then preforms a Leaky Relu with parameter 0.2.
    ...
    def __init__(self, dim, in_channels, out_channels, stride=1):
        ...
        Instiatiate the conv block
            ...
            :param stride: stride of the convolution
        """
        super(conv_block_dis, self).__init__()

        conv_fn = getattr(nn, "Conv{0}d".format(dim))
        ksize = 5
        self.main = conv_fn(in_channels, out_channels, ksize, stride, 2)
        self.activation = nn.LeakyReLU(0.2)
