# -*- coding: utf-8 -*-
"""
Created on Tue Jun 30 22:54:19 2020
...
#        moving_patch, fixed_patch = next(train_example_gen)
        for num_patch in range(100):
            ...
            flow, flow_up = model_x2(input_A, input_B)
            flow_up_x2 = flow_up * 2
            warp_x2 = ST(input_A, flow_up_x2)
...
def train_joint(
    ...
    sim_param_x2 = params_joint[0]
    sim_param_x1 = params_joint[1]
    grad_param = params_joint[2]
    reg_param = params_joint[3]
    
    model_parameters = list(model_x2.parameters()) + list(model_x1.parameters())
...
#        moving_patch, fixed_patch = next(train_example_gen)
        for num_patch in range(100):
            ...
            # Run the data through the model to produce warp and flow field
            # x2
            flow_x2, flow_up_x2 = model_x2(input_A, input_B)
            ...
            grad_loss = grad_param * grad_loss_fn(dvf)
            reg_loss = reg_param * reg_loss_fn(dvf)
            sim_loss = sim_loss_x2 + sim_loss_x1
            loss = sim_loss + grad_loss + reg_loss
            opt.zero_grad()
            loss.backward()  
            opt.step()
    ...
    return model_x2, model_x1, sim_loss_x2_out, sim_loss_x1_out, sim_loss_out, grad_loss_out, reg_loss_out
