...
#        warp_RTS = torch.zeros_like(src_RTS)
#        for i in range(src_RTS.size()[1]):
#            input_RTS = src_RTS[:,i,:,:,:]
#            input_RTS = torch.unsqueeze(input_RTS,0)
#            warp_RTS[0,i,:,:,:] = self.spatial_transform(input_RTS, flow)
        return y, flow

class cvpr2018_net_RTS(nn.Module):
    """
    [cvpr2018_net] is a class representing the specific implementation for 
    the 2018 implementation of voxelmorph.
    """
    def __init__(self, vol_size, enc_nf, dec_nf, full_size=True):
        ...
        Instiatiate 2018 model
            ...
            :param full_size: boolean value full amount of decoding layers
        """
        super(cvpr2018_net_RTS, self).__init__()
    ...
    def forward(self, src, RTS, tgt):
        ...
        flow = self.flow(x)
        y = self.spatial_transform(src, flow)
        warp_RTS = self.spatial_transform(RTS, flow)       
        
        return y, flow, warp_RTS
