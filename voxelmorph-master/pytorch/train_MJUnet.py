# -*- coding: utf-8 -*-
"""
Created on Wed Jun 10 20:23:13 2020
...
def train_initial(
    ...
    device = "cuda"
    if scale == 4:
        model = Unet(nf_enc, nf_dec, 4)
    elif scale == 2:
        model = Unet(nf_enc, nf_dec, 2)
    elif scale == 1:
        model = Unet(nf_enc, nf_dec, 1)
    else:
        print('Error! Scale has to be 4, 2, or 1.\n')
    ...
    opt_fine = Adam(model.parameters(), lr=lr)

    sim_loss_fn = losses.ncc_loss
    grad_loss_fn = losses.gradient_loss
    grad_loss_z_fn = losses.gradient_loss_z
    ...
    for i in range(n_epoch):
        ...
        opt_fine = Adam(model.parameters(), lr=lr_new)
        # Save model checkpoint
        if i % n_save_epoch == 0:
            save_file_name_fine = os.path.join(model_dir, 'model_%d_%d.ckpt' % (scale, i))
            torch.save(model.state_dict(), save_file_name_fine)
...
#        moving_patch, fixed_patch = next(train_example_gen)
        for num_patch in range(100):
            ...
            flow, flow_up = model(input_A, input_B)
...
#                mse_loss_f_out = np.append(mse_loss_f_out, np.mean(mse_loss_f_accu))
        ...
        reg_loss_out = np.append(reg_loss_out, np.mean(reg_loss_accu))
    return model,sim_loss_out,grad_loss_out, reg_loss_out
...
#        moving_patch, fixed_patch = next(train_example_gen)
        for num_patch in range(100):
            ...
            print("\r n_iter: %d/100, time = %fs, lr: %e, sim: %f + %f + %f = %f, grad:%f, reg:%f." 
              % (num_patch + 1, end-start, lr_new,np.mean(sim_loss_x4_accu),np.mean(sim_loss_x2_accu),np.mean(sim_loss_x1_accu), np.mean(sim_loss_accu),np.mean(grad_loss_accu),np.mean(reg_loss_accu)
                 ), end = '', flush=True)
        sim_loss_out = np.append(sim_loss_out, np.mean(sim_loss_accu))
        sim_loss_x4_out = np.append(sim_loss_x4_out, np.mean(sim_loss_x4_accu))
        sim_loss_x2_out = np.append(sim_loss_x2_out, np.mean(sim_loss_x2_accu))
