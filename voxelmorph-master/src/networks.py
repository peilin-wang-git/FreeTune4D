...
def unet_core(vol_size, enc_nf, dec_nf, full_size=True, src=None, tgt=None, src_feats=1, tgt_feats=1):
    ...
    # Dual inputs, one moving and one target image
...
def unet_core_MRI(vol_size, enc_nf, dec_nf, full_size=True, src=None, tgt=None, src_feats=1, tgt_feats=1):
    """
    unet architecture for voxelmorph models presented in the CVPR 2018 paper. 
    You may need to modify this code (e.g., number of layers) to suit your project needs.
    :param vol_size: volume size. e.g. (256, 256, 256)
    ...
    for i in range(len(enc_nf)):
        # inputs: input size, number of filters, stride
        # Default kernel size is 3x3
        x_enc.append(conv_block(x_enc[-1], enc_nf[i], 1))
        x_enc.append(conv_block(x_enc[-1], enc_nf[i], 2))
...
def unet_core_4x4x1(vol_size, enc_nf, dec_nf, full_size=True, src=None, tgt=None, src_feats=1, tgt_feats=1):
    """
    unet architecture for voxelmorph models presented in the CVPR 2018 paper. 
    You may need to modify this code (e.g., number of layers) to suit your project needs.
    :param vol_size: volume size. e.g. (256, 256, 256)
    ...
    x = conv_block(x, dec_nf[3])
    x = upsample_layer()(x)
    x = concatenate([x, x_enc[-9]])
    x = conv_block(x, dec_nf[4])
    x = conv_block(x, dec_nf[4])
    x = upsample_layer()(x)
    x = concatenate([x, x_enc[-11]])
    x = conv_block(x, dec_nf[5])
    x = conv_block(x, dec_nf[5])
    x = upsample_layer()(x)
    x = concatenate([x, x_enc[-13]])
    x = conv_block(x, dec_nf[6])
    x = conv_block(x, dec_nf[6])
    x = concatenate([x, x_enc[0]])
    x = conv_block(x, dec_nf[7])
    x = conv_block(x, dec_nf[7])
...
def cvpr2018_net(vol_size, enc_nf, dec_nf, full_size=True, indexing='ij'):
    ...
    # The '3' at last. No need to have [...,3]
...
def cvpr2018_net_lambda(vol_size, enc_nf, dec_nf, full_size=True, indexing='ij'):
    """
    unet architecture for voxelmorph models presented in the CVPR 2018 paper. 
    You may need to modify this code (e.g., number of layers) to suit your project needs.
    ...
    # get the core model
    unet_model = unet_core_4x4x1(vol_size, enc_nf, dec_nf, full_size=full_size)
    [src, tgt] = unet_model.inputs
    x = unet_model.output
...
def cvpr2018_net_MRI(vol_size, enc_nf, dec_nf, full_size=True, indexing='ij'):
    """
    unet architecture for voxelmorph models presented in the CVPR 2018 paper. 
    You may need to modify this code (e.g., number of layers) to suit your project needs.
