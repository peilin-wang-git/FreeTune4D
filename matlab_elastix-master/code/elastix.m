...
if ~strcmp(outputDir,'.')
    if ~movefile([movingFname,'.mhd'],outputDir); error('Can''t move files'), end
end
if ~strcmp(outputDir,'.')
    if ~movefile([movingFname,'.raw'],outputDir); error('Can''t move files'), end
end
...
if isnumeric(fixedImage)
    ...
    if ~strcmp(outputDir,'.') %Don't copy if we're already in the directory
        if ~movefile([targetFname,'.mhd'],outputDir); error('Can''t move files'), end
    end
    if ~strcmp(outputDir,'.') %Don't copy if we're already in the directory
        if ~movefile([targetFname,'.raw'],outputDir); error('Can''t move files'), end
    end
end

%Build the parameter file(s)
%modify settings from YAML with paramstruct
fprintf('Now is building the parameterfile.\n')
if ~isempty(paramstruct) && (ischar(paramFile) && endsWith(paramFile,'.yml')) || (isnumeric(paramFile) && paramFile==-1) 
    fprintf('entered yml path 1.\n')
...
elseif iscell(paramFile) %we have a cell array of elastix parameter files
    fprintf('entered cell path.\n')
    paramFname = paramFile;
     if ~strcmp(outputDir,'.') 
       %save('E:\4DMRISlicer\slicer\PhaseSorting\PhaseSorting\RegistrationSrc\debug.mat','paramFile','outputDir')
        for ii=1:length(paramFname)
            fprintf('paramFile is %s\n', paramFname{ii})
            fprintf('outputDir is %s\n', outputDir)
